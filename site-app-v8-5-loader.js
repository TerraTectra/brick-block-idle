(() => {
  ...
  function patchRuntime(source) {
    ...
    code = code.replace(
      ...
      'const RUN_KEY = "brick_block_idle_run_v8_5";',
    );
    ...
    code = code.replace(
      ...
      'const state = { level: 1, fragments: 0, total: 0, paused: false, bricks: [], balls: [], upgrades: { damage: 1, speed: 0, xp: 0 }, log: ["v8.5: скорость сохраняется между этапами."] };',
    );
    ...
    code = code.replace(
      ...
      'function xpGain(brick, ball) { if (isEvolutionLocked(ball)) return 0; const bonus = ball.form === "collector" ? 1 + ball.evo * 0.06 : 1; const stageXpBonus = 1 + Math.pow(Math.max(1, state.level), 0.45) * 0.035; return Math.max(1, Math.ceil(brick.xp * 0.08 * xpMult() * bonus * stageXpBonus)); }',
    );
  ...
  bindUi(); ui(); renderMeta(); loop();`,
    ...
    return code + "\n//# sourceURL=site-app-v8-5-runtime.js";
  }
  ...
  fetch(SOURCE, { cache: "no-store" })
    ...
    .catch((error) => {
      console.error("Ошибка запуска v8.5:", error);
      ...
      box.textContent = `Ошибка запуска игры v8.5:\n${error.message}`;
