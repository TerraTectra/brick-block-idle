(() => {
  ...
  function patchRuntime(source) {
    ...
    code = code.replace(
      ...
      'const RUN_KEY = "brick_block_idle_run_v8_6";',
    );
    ...
    code = code.replace(
      ...
      'const forms = [\n    { key: "plasma", name: "Плазма", color: "#ff4d6d", damage: 0.9, speed: 0.95, radius: 1.05, desc: "Процентный урон по области. Медленнее базового, но сильна против жирных блоков." },\n    { key: "storm", name: "Гроза", color: "#f9f871", damage: 0.8, speed: 1.18, radius: 0.95, desc: "Быстрый шар. После добивания цепляет молнией несколько блоков." },\n    { key: "drill", name: "Бур", color: "#ff9f1c", damage: 1.35, speed: 0.72, radius: 0.9, desc: "Тяжёлый медленный шар. Сильно пробивает броню, щиты и тяжёлые блоки." },\n    { key: "crit", name: "Крит", color: "#b517ff", damage: 1.05, speed: 1.0, radius: 1.0, desc: "Сбалансированный шар с шансом мощного критического удара." },\n    { key: "comet", name: "Комета", color: "#2ec4b6", damage: 0.92, speed: 1.32, radius: 0.85, desc: "Очень быстрый лёгкий шар. Скорость превращается в дополнительный урон." },\n    { key: "collector", name: "Сборщик", color: "#60a5fa", damage: 0.7, speed: 1.05, radius: 1.0, desc: "Слабее в бою, но получает больше опыта и осколков." },\n    { key: "poison", name: "Ядовитый", color: "#84cc16", damage: 0.78, speed: 0.9, radius: 1.05, desc: "Накладывает яд. Блоки получают периодический урон после попадания." }\n  ];',
    );
    ...
    code = code.replace(
      ...
      'const state = { level: 1, fragments: 0, total: 0, paused: false, bricks: [], balls: [], upgrades: { damage: 1, speed: 0, xp: 0 }, log: ["v8.6: шаровые архетипы и усиление эволюций."] };',
    );
    ...
    code = code.replace(
      ...
      'function formStats(ball) { return forms.find((item) => item.key === ball.form) || { damage: 1, speed: 1, radius: 1 }; }\n  function damageFor(ball) { const stats = formStats(ball); const base = 1 + (ball.level - 1) * 0.25 + Math.pow(ball.level - 1, 1.05) * 0.045; const evo = 1 + ball.evo * 0.16 + Math.pow(ball.evo, 1.18) * 0.03; return +(base * evo * (stats.damage || 1) * upgradePower(effectiveUpgrade("damage"), "damage")).toFixed(2); }',
    );
    ...
    code = code.replace(
      ...
      'function applyEvolution(ballId, formKey) { const ball = state.balls.find((item) => item.id === ballId); const form = forms.find((item) => item.key === formKey); if (!ball || !form) return; const milestone = ball.pendingEvolution || pendingMilestone(ball); if (!milestone) return; if (ball.form !== "basic") { const current = forms.find((item) => item.key === ball.form) || form; ball.evo += 1; ball.evolved.push(milestone); ball.pendingEvolution = null; ball.xp = 0; ball.need = xpNeed(ball.level); ball.r = 9 + Math.min(12, ball.evo * 0.95) * (formStats(ball).radius || 1); ball.dmg = damageFor(ball); log(`Шар-${ball.id}: усилена форма ${current.name}.`); return; } ball.evo += 1; ball.form = form.key; ball.formName = form.name; ball.color = form.color; ball.r = 9 + Math.min(12, ball.evo * 0.95) * (form.radius || 1); ball.evolved.push(milestone); ball.pendingEvolution = null; ball.xp = 0; ball.need = xpNeed(ball.level); ball.dmg = damageFor(ball); log(`Шар-${ball.id} выбрал архетип: ${form.name}.`); }',
    );
    ...
    code = code.replace(
      ...
      'function showEvolution(ballId) { const ball = state.balls.find((item) => item.id === ballId); if (!ball) return; const milestone = ball.pendingEvolution || pendingMilestone(ball); if (!milestone) return; if (ball.form !== "basic") { applyEvolution(ball.id, ball.form); ui(); return; } if (metaEffects().autoEvolution) { applyEvolution(ball.id, chooseThreeForms(ball)[0].key); return; } ball.pendingEvolution = milestone; const choices = chooseThreeForms(ball); document.querySelector(".evo-modal")?.remove(); const modal = document.createElement("div"); modal.className = "evo-modal"; modal.innerHTML = `<div class="evo-box"><h2>Архетип Шара-${ball.id}</h2><p>Шар достиг ${milestone} уровня. Выбери базовую специализацию. Следующие эволюции будут усиливать её.</p><div class="evo-grid">${choices.map((form) => `<button class="evo-choice" data-ball="${ball.id}" data-form="${form.key}"><span class="dot" style="background:${form.color}"></span><b>${form.name}</b><small>${form.desc}<br>Урон x${form.damage} · Скорость x${form.speed}</small></button>`).join("")}</div><button class="control evo-close">Позже</button></div>`; modal.addEventListener("pointerdown", (event) => { const choice = event.target.closest(".evo-choice"); if (choice) { applyEvolution(Number(choice.dataset.ball), choice.dataset.form); modal.remove(); ui(); return; } if (event.target.closest(".evo-close")) modal.remove(); }); document.body.appendChild(modal); }',
    );
    ...
    code = code.replace(
      ...
      'function hitDamage(ball, brick) { let amount = ball.dmg; if (ball.form === "plasma") amount += Math.max(1, brick.max * (0.015 + ball.evo * 0.006)); if (ball.form === "crit" && Math.random() < clamp(0.1 + ball.evo * 0.03, 0.1, 0.42)) amount *= 1.55 + ball.evo * 0.12; if (ball.form === "drill" && brick.hp / brick.max > 0.5) amount *= 1.18 + ball.evo * 0.1; if (ball.form === "comet") amount *= 1 + Math.min(0.75, (personalSpeed(ball) - 1) * 0.22); return amount; }',
    );
  ...
  bindUi(); ui(); renderMeta(); loop();`,
    ...
    return code + "\n//# sourceURL=site-app-v8-6-runtime.js";
  }
  ...
  fetch(SOURCE, { cache: "no-store" })
    ...
    .catch((error) => {
      console.error("Ошибка запуска v8.6:", error);
      ...
      box.textContent = `Ошибка запуска игры v8.6:\n${error.message}`;
