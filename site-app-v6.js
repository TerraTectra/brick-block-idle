(() => {
  const W = 980;
  const H = 650;
  const SAVE_KEY = "brick_block_idle_static_v6";
  ...
  const forms = [
    ...
    { key: "collector", name: "Сборщик", color: "#60a5fa", desc: "Получает больше опыта и осколков за добивания." },
  ];
  const types = [
  ...
  const state = {
    ...
    meta: { autoEvolution: false },
    log: ["v6: ручная эволюция включена."],
  };
  let frame = 0;
  ...
  function log(text) { state.log = [text, ...state.log].slice(0, 8); }
  function activeTypes() { return types.filter((t) => state.level >= t.min); }
  ...
  function pendingMilestone(ball) {
    return milestones.find((m) => ball.level >= m && !ball.evolved.includes(m)) || null;
  }
  ...
  function totalSpeed(ball) { return personalSpeed(ball) * upgradePower(state.upgrades.speed, "speed"); }
  function xpMult() { return upgradePower(state.upgrades.xp, "xp"); }
  function xpGain(brick, ball) {
    if (isEvolutionLocked(ball)) return 0;
  ...
  function ballCost() { return Math.floor(320 * Math.pow(2.15, Math.max(0, state.balls.length - 2))); }
  ...
  function chooseThreeForms(ball) {
    const random = rng(Date.now() + ball.id * 1337 + ball.level * 17);
    const pool = [...forms];
    const result = [];
  ...
  function showEvolution(ballId) {
    ...
    const choices = chooseThreeForms(ball);
    document.querySelector(".evo-modal")?.remove();
    const modal = document.createElement("div");
    modal.className = "evo-modal";
    modal.innerHTML = `
      <div class="evo-box">
        <h2>Эволюция Шара-${ball.id}</h2>
        <p>Шар достиг ${milestone} уровня. Опыт остановлен до выбора формы.</p>
        <div class="evo-grid">
          ${choices.map((form) => `
            <button class="evo-choice" data-ball="${ball.id}" data-form="${form.key}">
              <span class="dot" style="background:${form.color}"></span>
              <b>${form.name}</b>
              <small>${form.desc}</small>
            </button>
          `).join("")}
        </div>
        <button class="control evo-close">Позже</button>
      </div>
    `;
  ...
  function applyEvolution(ballId, formKey) {
    ...
    ball.need = xpNeed(ball.level);
    ball.dmg = damageFor(ball);
    log(`Шар-${ball.id} эволюционировал: ${form.name}.`);
  }
  function reward(ball, brick) {
    state.fragments += fragGain(brick, ball);
    state.total += 1;
    ball.kills += 1;
    if (isEvolutionLocked(ball)) return;
    ball.xp += xpGain(brick, ball);
    while (!isEvolutionLocked(ball) && ball.xp >= ball.need) {
      ball.xp -= ball.need;
      ball.level += 1;
      ball.need = xpNeed(ball.level);
      ball.dmg = damageFor(ball);
      log(`Шар-${ball.id}: ${ball.level} ур.`);
      const milestone = pendingMilestone(ball);
      if (milestone) {
        ball.pendingEvolution = milestone;
        ball.xp = 0;
        log(`Шар-${ball.id} ждёт эволюцию ${milestone} ур.`);
        break;
      }
    }
  }
  ...
  function draw() {
    ...
    for (let x = 22; x < W; x += 42) for (let y = 76; y < H; y += 42) { ctx.beginPath(); ctx.arc(x, y, 1.45, 0, Math.PI * 2); ctx.fill(); }
    rr(22, 18, W - 44, 42, 16);
    ...
    for (const ball of state.balls) {
      ...
      ctx.fill();
      ctx.strokeStyle = ball.pendingEvolution ? "#facc15" : "#fff";
      ctx.lineWidth = ball.pendingEvolution ? 3 : 1;
      ctx.stroke();
      ctx.lineWidth = 1;
  ...
  function save() {
    ...
    localStorage.setItem(SAVE_KEY, JSON.stringify({ level: state.level, fragments: state.fragments, total: state.total, upgrades: state.upgrades, meta: state.meta, balls: state.balls }));
  }
  function load() {
    try {
  ...
  function ballCard(ball) {
    ...
    return `<div class="ball-card ${locked ? "evo-needed" : ""}">
      <b>Шар-${ball.id}</b>
      ...
      ${locked ? `<button class="evo-btn" data-evolve-ball="${ball.id}">Эволюционировать</button>` : ""}
    </div>`;
  }
  function ui() {
    ...
    el("balls").innerHTML = state.balls.map(ballCard).join("");
