(() => {
  ...
  function patchRuntime(source) {
    ...
    code = code.replace(
      ...
      'const state = { level: 1, fragments: 0, total: 0, paused: false, bricks: [], balls: [], upgrades: { damage: 1, speed: 0, xp: 0 }, log: ["v8.3: исправлен застревающий удар по углам блоков."] };',
    );
  ...
  bindUi(); ui(); renderMeta(); loop();`,
    ...
    return code + "\n//# sourceURL=site-app-v8-3-runtime.js";
  }
  ...
  fetch(SOURCE, { cache: "no-store" })
    ...
    .catch((error) => {
      console.error("Ошибка запуска v8.3:", error);
      ...
      box.textContent = `Ошибка запуска игры v8.3:\n${error.message}`;
