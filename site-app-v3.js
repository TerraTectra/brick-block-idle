(() => {
  const W = 980;
  const H = 650;
  const SAVE_KEY = "brick_block_idle_static_v3";
  ...
  function upgradePower(level) {
    if (level <= 0) return 1;
    return +(1 + level + (Math.pow(level, 1.55) - level) * 0.45).toFixed(2);
  }
  function xpNeed(level) {
    if (level <= 1) return 10;
    return Math.floor(10 + level * 7 + Math.pow(level, 1.72) * 8 + Math.pow(Math.max(0, level - 10), 2.12) * 14 + (milestones.includes(level) ? level * 28 : 0));
  }
  ...
  function totalSpeed(ball) { return personalSpeed(ball) * upgradePower(state.upgrades.speed); }
  function xpMult() { return upgradePower(state.upgrades.xp); }
  function ballCost() { return Math.floor(150 * Math.pow(1.7, Math.max(0, state.balls.length - 2))); }
  ...
  function makeBall(id, index, saved) {
    ...
    if (typeof ball.dx !== "number" || typeof ball.dy !== "number") {
      ball.dx = typeof ball.vx === "number" ? ball.vx : Math.cos(angle);
      ball.dy = typeof ball.vy === "number" ? ball.vy : -Math.abs(Math.sin(angle));
    }
    normalizeDir(ball);
  ...
  function reward(ball, brick) {
    ...
    state.fragments += Math.ceil(brick.xp * bonus);
    state.total += 1;
    ball.kills += 1;
    ball.xp += Math.ceil(brick.xp * xpMult() * bonus);
    while (ball.xp >= ball.need) {
      ball.xp -= ball.need;
      ball.level += 1;
      ball.need = xpNeed(ball.level);
      ball.dmg = damageFor(ball);
      log(`Шар-${ball.id}: ${ball.level} ур. Скорость x${totalSpeed(ball).toFixed(2)}.`);
  ...
  function hitDamage(ball, brick) {
    let amount = ball.dmg;
    if (ball.form === "crit" && Math.random() < clamp(0.1 + ball.evo * 0.035, 0.1, 0.48)) amount *= 1.8 + ball.evo * 0.18;
    if (ball.form === "drill" && brick.hp / brick.max > 0.5) amount *= 1.25 + ball.evo * 0.14;
    if (ball.form === "comet") amount *= 1 + Math.min(0.95, (personalSpeed(ball) - 1) * 0.28);
    return amount;
  }
  ...
  function step() {
    ...
    for (const ball of state.balls) {
      const move = 2.65 * totalSpeed(ball);
      ball.x += ball.dx * move;
      ball.y += ball.dy * move;
      ...
      for (let i = state.bricks.length - 1; i >= 0; i -= 1) {
        ...
        if (ball.form === "storm" && destroyed && state.bricks.length > 0) {
          ...
          for (let n = 0; n < jumps && state.bricks.length > 0; n += 1) dealDamage(Math.floor(Math.random() * state.bricks.length), ball.dmg * (0.62 + ball.evo * 0.08), ball, "chain");
        }
        break;
      }
    }
    if (state.bricks.length === 0) {
      state.level += 1;
      generateBricks();
      for (const ball of state.balls) normalizeDir(ball);
      log(`Открыт этап ${state.level}. Скорость шаров сохранена.`);
    }
  }
  ...
  function ui() {
    ...
    document.getElementById("types").innerHTML = activeTypes().map((type) => `<small><span class="dot" style="background:${type.color}"></span>${type.name}</small>`).join("");
    document.getElementById("balls").innerHTML = state.balls.map((ball) => `<div class="ball-card"><b>Шар-${ball.id}</b><small>Ур. ${ball.level} · Урон ${ball.dmg} · Скорость x${totalSpeed(ball).toFixed(2)} · XP ${Math.floor(ball.xp)}/${ball.need}</small></div>`).join("");
  ...
  function bindUi() {
    document.getElementById("pauseBtn").onclick = () => { state.paused = !state.paused; ui(); };
  ...
  function loop() { step(); draw(); ui(); save(); requestAnimationFrame(loop); }
